...
class MetaTaskDataset(Dataset):
    """从 base_dataset 中按时间窗口采样多条轨迹的片段，生成支持集和查询集。"""
    def __init__(self, base_dataset, num_tasks=10000, k_shot=5, q_shot=5, time_window=900, device='cuda'):
        super().__init__()
        self.base_dataset = base_dataset
        self.num_tasks = num_tasks
        self.k_shot = k_shot
        self.q_shot = q_shot
        self.time_window = time_window  # 时间窗口大小，单位：秒，默认为15分钟
        self.device = device

        # 预处理：按时间窗口组织轨迹点
        self.time_windows = self._organize_by_time_window()

    def _organize_by_time_window(self):
        """将所有轨迹点按时间窗口分组，并计算 delta_t。"""
        all_points = []
        for traj_id in self.base_dataset.traj_ids:
            ...
            for _, row in traj.iterrows():
                all_points.append({
                    'traj_id': traj_id,
                    'timestamp': row[T_COL],  # 使用 T_COL 作为时间戳
                    'features': row[[X_COL, Y_COL, T_COL, DT_COL, ROAD_COL]].to_numpy(dtype=np.float64)
                })
        ...
        for point in all_points:
            window_key = int(point['timestamp'] // self.time_window)
            if window_key not in time_windows:
                time_windows[window_key] = []
            time_windows[window_key].append(point)
    ...
    def __getitem__(self, index):
        """返回一个任务的支持集和查询集，包含多条轨迹的片段。"""
        # 随机选择一个时间窗口
        window_keys = list(self.time_windows.keys())
        selected_window = np.random.choice(window_keys)
        points_in_window = self.time_windows[selected_window]
        
        # 确保窗口内有足够的点
        while len(points_in_window) < (self.k_shot + self.q_shot):
            selected_window = np.random.choice(window_keys)
            points_in_window = self.time_windows[selected_window]
        
        # 随机采样 k_shot + q_shot 个点
        sampled_points = np.random.choice(points_in_window, size=self.k_shot + self.q_shot, replace=False)
        
        # 分割为支持集和查询集
        support_points = sampled_points[:self.k_shot]
        query_points = sampled_points[self.k_shot:]
        
        # 优化：先将列表转换为 NumPy 数组，再转为 Tensor
        support_features = np.array([p['features'] for p in support_points], dtype=np.float64)  # 形状 (k_shot, 5)
        query_features = np.array([p['features'] for p in query_points], dtype=np.float64)      # 形状 (q_shot, 5)
        
        support_tensor = torch.tensor(support_features, dtype=torch.float).to(self.device)
        query_tensor = torch.tensor(query_features, dtype=torch.float).to(self.device)
    ...
    for setting_i, setting in enumerate(settings):
        ...
        if 'pretrain' in setting:
            ...
            if setting['pretrain'].get('load', False):
                ...
                if setting['pretrain'].get('maml', False):  # MAML模式
                    # 创建元任务数据集
                    #成都时间步长为900秒，然而西安需要另外设置时间步长time_window，由于西安轨迹数据集更稀疏，所以时间步长设置为5400秒
                    meta_dataset = MetaTaskDataset(
                        ...
                        time_window=5400,  # 15分钟 ,新增时间窗口参数
                        device=device
                    )
        ...
        if 'test' in setting:
            ...
            if setting['test'].get('save', False):
                ...
                # 创建以城市为基础的保存目录
                city_dir = os.path.join(PRED_SAVE_DIR,'meta','仅时间的', city_name, SAVE_NAME, down_task)  # 创建城市文件夹 + 任务文件夹
