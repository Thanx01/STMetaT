...
class MetaTaskDataset(Dataset):
    ...
    def __init__(self, base_dataset, num_tasks=10000, k_shot=5, q_shot=5, time_window=900, grid_size=0.014, device='cuda'):
        super().__init__()
        ...
        self.grid_size = grid_size      # 空间网格大小，例如 0.014 度（约 1km）
        self.device = device
            ...
            if setting['pretrain'].get('load', False):
                ...
                if setting['pretrain'].get('maml', False):  # MAML模式
                    ...
                    if setting['pretrain'].get('save', True):
                        ...
                        PRETRAIN_SAVE_NAME = f"{SAVE_NAME}_pretrain_{datetime_key}_Ptrajm_maml_test——02_grid_size=0.014"
        ...
        if 'finetune' in setting:
            ...
            if setting['finetune'].get('load', False):
                ...
                if setting['finetune'].get('save', True):
                    utils.create_if_noexists(MODEL_CACHE_DIR)
                    FINETUNE_SAVE_NAME = f"{SAVE_NAME}_finetune_{datetime_key}_Ptrajm——02_grid_size=0.014"
