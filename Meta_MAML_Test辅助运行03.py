...
parser.add_argument('--cuda', help='index of the cuda device to use', type=int, default='3')
...
class MetaTaskDataset(Dataset):
    ...
    def __init__(self, base_dataset, num_tasks=10000, k_shot=5, q_shot=5, time_window=900, grid_size=0.012, device='cuda'):
        super().__init__()
        ...
        self.grid_size = grid_size      # 空间网格大小，例如 0.012 度（约 1km）
        self.device = device
        ...
        if 'pretrain' in setting:
            ...
            if setting['pretrain'].get('load', False):
                ...
                if setting['pretrain'].get('maml', False):  # MAML模式
                    ...
                    time_window = 2700 if city_name == 'chengdu' else 2700  # 成都 900 秒，西安 2700 秒
                    ...
                    if setting['pretrain'].get('save', True):
                        ...
                        PRETRAIN_SAVE_NAME = f"{SAVE_NAME}_pretrain_{datetime_key}_Ptrajm_maml_test——03_grid_size=0.012"
        ...
        if 'finetune' in setting:
            ...
            if setting['finetune'].get('load', False):
                ...
                if setting['finetune'].get('save', True):
                    utils.create_if_noexists(MODEL_CACHE_DIR)
                    FINETUNE_SAVE_NAME = f"{SAVE_NAME}_finetune_{datetime_key}_Ptrajm——03_grid_size=0.012"
